...
Host machines, virtual machines, and local services produce useful diagnostic logs, but those logs are scattered across terminals, Docker, Windows, and device-specific locations. An agent can help summarize and connect those logs to work notes, but raw logs should not be pasted into chat or dumped into the vault.

## Desired Outcome

Create a local log inbox that accepts logs from many producers, stores them durably, exposes review tools to an agent, and lets the agent write concise summaries into the Obsidian vault when there is a durable result.

## Non-Goals

- Do not use MCP as the device ingestion protocol.
- Do not write raw log streams directly into Obsidian.
