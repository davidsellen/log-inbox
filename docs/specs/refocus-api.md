...
`GET /api/v2/settings/agent-guidance` requires `logs:read` and returns the workspace's recommended metadata fields, the supported field catalog, a copyable instruction snippet, and bounded coverage grouped by source for up to 500 events from the last 30 days. Coverage is advisory; missing values never block ingestion or Daily generation. `PUT` requires `settings:write` and CSRF and saves a unique subset of supported fields to the active workspace. Log Inbox does not edit agent-local instruction files.
...
`PUT|DELETE /api/v2/daily/{date}/evidence/{event_id}/metadata` requires `review:write` and CSRF. PUT saves a workspace-scoped overlay for the selected event's `task_id`, otherwise `session_id`, otherwise that event alone. DELETE removes that field's overlay for the same scope. The daily evidence response preserves received `metadata` and adds `effective_metadata`; overlays inform future generation but never rewrite ingested events or existing revisions. Overlays are anchored to retained evidence and expire with that source event.
...
Requires both `logs:read` and `knowledge:read`. It returns the immutable Knowledge provenance for the current Daily revision, grouped by its workstream IDs, plus a live status: `current`, `changed`, `invalid`, `unavailable`, or `none`. Each note states whether its frozen link is still attached to the current structured revision. For a locally generated bounded-context revision, it also returns only the exact excerpt frozen and sent for that note; link-only revisions expose no note text. The detail never returns other note content, aliases, frontmatter, selectors, collection roots, or event messages.
