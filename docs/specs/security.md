...
The Daily service refuses to start without an owner secret of at least 20 bytes. Login stores an Argon2 verifier and issues independently generated session and CSRF credentials. Only credential digests are stored. A valid session can recover a stable, domain-separated CSRF token from its opaque cookie after reload, without invalidating other tabs; the original login token remains valid for that session. The browser never persists the owner secret or CSRF token. Sessions carry explicit scopes and are revoked when the owner secret changes. Standard sessions have a 30-minute idle and eight-hour absolute expiry; explicitly remembered device sessions last up to 30 days.
