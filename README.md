...
Chrome and Edge users may instead choose an existing or empty vault directly from **Linking → Browse vault**. The browser retains the directory permission, synchronizes a derived catalog with the local service, and can add a previewed developer starter or Markdown-only template structure without overwriting existing files. The mounted configuration remains available for unattended and headless operation.
